...
path_ENA_origin = os.path.abspath(os.path.join(source_dir, os.pardir)) + "/16S_2_csv/Results/notnull_superkingdom/ENA.xlsx"
path_GTDB_origin = os.path.abspath(os.path.join(source_dir, os.pardir)) + "/16S_2_csv/Results/notnull_superkingdom/GTDB.xlsx"
path_LTP_origin = os.path.abspath(os.path.join(source_dir, os.pardir)) + "/16S_2_csv/Results/notnull_superkingdom/LTP.xlsx"
path_NCBI_origin = os.path.abspath(os.path.join(source_dir, os.pardir)) + "/16S_2_csv/Results/notnull_superkingdom/NCBI.xlsx"
path_SILVA_origin = os.path.abspath(os.path.join(source_dir, os.pardir)) + "/16S_2_csv/Results/notnull_superkingdom/SILVA.xlsx"
...
for index, value in df1_col.iteritems():
    if value in df4_col.values and value in df5_col.values and value: #in df2_col.values and value in df5_col.values and value:
        df_result = df_result.append(df1.loc[index], ignore_index=True)

result_df = pd.DataFrame(df_result)
result_df.to_csv("C:/Users/fchio/Desktop/GroupProject/GP-Chiocchi-Filipponi-Martini/16S_2_csv/Results/notnull_phylum/join_notnull_phylum.csv", index=False)